...
def plot_spread(dQx, dQy, normalise=True):
    ...
    ax = sbs.jointplot(x=dQx/dQrms_x, y=dQy/dQrms_y, kind='hex', color=palette[-2], marginal_kws={'bins': 25,}, ratio=3 , xlim=(-0.1, 7), ylim=(-7, .1))
    ax.ax_joint.plot(np.mean(dQxRFQ)/dQrms_x, np.mean(dQyRFQ)/dQrms_y, marker='o', c='darkgreen', label='RFQ coherent tune shift')
