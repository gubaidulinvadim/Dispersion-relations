...
def plot_spread(dQx, dQy, filename=None, normalise=True):
    ...
    ax.ax_joint.set_xlabel(r'$\Delta Q_x/\Delta Q_\mathrm{RMS}$') #/ \delta Q_{rms}$')
    ax.ax_joint.set_ylabel(r'$\Delta Q_y/\Delta Q_\mathrm{RMS}$') #/ \delta Q_{rms}$')
...
if __name__ == '__main__':
    ...
    ax.ax_joint.plot(np.mean(dQxel)/rms_x, np.mean(dQyel)/rmx_y, marker='o', c='r')
