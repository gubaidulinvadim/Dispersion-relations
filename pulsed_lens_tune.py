...
def plot_spread(dQx, dQy, normalise=True):
    ...
    ax = sbs.jointplot(x=dQx/dQrms_x, y=dQy/dQrms_y, kind='hex', color=palette[-1], marginal_kws={'bins': 25,}, ratio=3 , xlim=(-0.1, 7.), ylim=(-.1, 7.))
    ax.ax_joint.plot(np.mean(dQx)/dQrms_x, np.mean(dQy)/dQrms_y, marker='o', c='darkgreen', label='PEL coherent tune shift')
...
if __name__ == '__main__':
    ...
    dQxPEL, dQyPEL = get_pelens_tune(Jz, max_tune_shift_x=1e-3, max_tune_shift_y=1e-3)
